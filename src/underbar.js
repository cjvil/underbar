(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(allMatch, element) {
      if (allMatch) {
        allMatch = iterator === undefined ? element : Boolean(iterator(element));
      }
      return allMatch;
    }, true);
  };
  ...
  _.extend = function(obj) {
    _.each(Array.from(arguments).slice(1), function(element) {
  ...
  _.shuffle = function(array) {
    var randomized = array.slice(0);
    var temp;

    for(var i = 0; i < randomized.length - 1; i++) {
      var random = Math.floor(Math.random() * (randomized.length - i)) + i;
      temp = randomized[i];
  ...
  _.zip = function() {
    ...
    _.each(Array.from(arguments), function(arg) {
      _.each(arg, function(element, index) {
        zipped[index] === undefined ? zipped[index] = [element] : zipped[index].push(element);
      });
    });

    _.each(zipped, function(element) {
      while (element.length < arguments.length) {
  ...
  _.intersection = function() {
    ...
    _.each(arguments[0], function(element) {
      var include = _.every(otherArrays, function(arr) {
  ...
  _.difference = function(array) {
    ...
    return _.filter(array, function(element) {
      return _.every(otherArrays, function(arr) { 
        if(arr.includes(element)) {
          return false;
        }
        return true;
